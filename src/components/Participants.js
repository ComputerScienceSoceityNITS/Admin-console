...
const RowParticipants = ({
  ...
  return (
   <div key={data.id}>
    <div className={clickedRow ? "p_row expand_p" : "p_row shrink_p"} key={data.id} onClick={()=> setClickedRow(!clickedRow)} draggable title="Expandable on click" >
        ...
        <p title="teamLeader_schid">Scholar Id : {data.teamLeader.scholarID}</p>
        </>)
        :""}
